...
export class ProveedoresComponent implements OnInit {
  ...
  form: Proveedor = { 
    ...
    email: '', direccion: '', categoria: 'General' 
  };
  ...
  validarRuc(ruc: string): boolean {
    ...
    if (!ruc.endsWith('001')) return false; // RUCs generalmente terminan en 001

    // Validamos los primeros 10 dígitos como cédula o ruc jurídico
    const provincia = parseInt(ruc.substring(0, 2));
    if (provincia < 1 || provincia > 24) return false;
    
    // Por simplicidad en tarea académica, validamos longitud y provincia.
    // El algoritmo módulo 11 completo para sociedades es muy extenso, 
    // pero con esto filtramos los errores obvios.
    return true; 
  }
  ...
  guardar() {
    ...
    if (!this.telefonoRegex.test(this.form.telefono)) {
  ...
  cancelar() {
    this.editando = false;
    this.form = { ruc: '', nombreEmpresa: '', contactoNombre: '', telefono: '', email: '', direccion: '', categoria: 'General' };
  }
}